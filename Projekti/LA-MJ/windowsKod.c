...
List* insert(List* head, const char* word) {
    List* current = head;
    while (current != NULL) {
    ...
    newNode->freq = 1;
    newNode->sledeci = head;
    return newNode;
}
List* processString(char* input) {
    List* head = NULL;
    char* token;
...
void insertPipe(Pipe **kraj, int x){
  Pipe* novi=(Pipe*)malloc(sizeof(Pipe));
  novi->sledeci=NULL;
  novi->podatak=x;
  (*kraj)->sledeci=novi;
...
Head* ReadHead(Head* root, List* lista, char passdown[], int i, int ascii);
...
Head* ReadHead(Head* koren, List* lista, char passdown[], int i, int ascii){
  Head* root=koren;
...
int main(){
  ...
  List* popravljena=processString(input);
